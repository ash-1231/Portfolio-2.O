...
const FloatingDockDesktop = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "mx-auto hidden h-16 items-end gap-4 rounded-2xl bg-gray-950 px-4 pb-3 md:flex dark:bg-neutral-900",
        className,
      )}
    >
      {items.map((item) => (
  ...
  return (
    <a href={href}>
      <motion.div
        ...
        className="relative flex items-center justify-center rounded-full bg-slate-800 px-4 text-sm font-medium text-indigo-300 dark:bg-neutral-800 dark:text-gray-200"
      >
        <AnimatePresence>
          {hovered && (
            <motion.div
              ...
              className="absolute -top-8 left-1/2 w-fit rounded-md border border-gray-200 bg-gray-100 px-2 py-0.5 text-xs text-neutral-700 dark:border-neutral-700 dark:bg-neutral-800 dark:text-white"
            >
              {title}
            </motion.div>
          )}
